...
export const SCHEMA = {
  ...
  mesh: {
    ...
    fields: {
      ...
      vrm: {
        ...
        fields: {
          src: { doc: 'avatar file URL; templates in /assets/vrm/ (nyari-final, cand-*)' },
          edits: {
            doc: 'live customization: { colors: {slot: hex} (slots: iris/hair/hairBack/faceSkin/bodySkin/sclera/eyeline/eyelash/brow/mouth/tops/bottoms/shoes/neckAccessory), expressions: {happy|angry|sad|relaxed|surprised|aa..oh|blink: 0..1}, bones: {humanoidBone: scale}, nodes: {nameSubstring: scale — raw bones e.g. Bust/Skirt}, meta: {title, author — baked on export} }',
          },
  ...
  characterCreator: {
    doc: 'clean-room character-creator recipe for rebuilding a humanoid mesh from sliders and colors; no proprietary source assets',
    default: {
      version: 1,
      source: 'clean-room-gaia',
      skin: '#f1c7a8',
      hair: '#2b1738',
      iris: '#7a1128',
      outfit: '#26344f',
    ...
    fields: {
      source: { doc: 'asset lineage; built-in creator writes clean-room-gaia' },
      skin: { doc: 'skin color' },
      hair: { doc: 'hair color' },
      iris: { doc: 'iris color' },
      outfit: { doc: 'outfit color' },
      height: { doc: 'overall body height multiplier', range: [0.65, 1.45] },
      head: { doc: 'head size multiplier', range: [0.7, 1.55] },
      eye: { doc: 'eye size multiplier', range: [0.55, 1.7] },
      hairLength: { doc: 'back-hair mass/length blend', range: [0, 1] },
      shoulders: { doc: 'shoulder width multiplier', range: [0.65, 1.45] },
      hips: { doc: 'hip width multiplier', range: [0.65, 1.45] },
    },
  },
  light: {
