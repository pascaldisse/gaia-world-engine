#!/usr/bin/env bash
# GESTURE-FIX LANE · the ONLY way this lane starts anything.
#
# IRON PORTS (this lane, never shared): world 8462 · client 5197 · quest 4695.
...
cd "$(dirname "$0")/.."
ROOT=$(pwd)
SCRATCH="$ROOT/.scratch/gesture-fix"
WORLD="$SCRATCH/world"
QDATA="$SCRATCH/quest"
GAIA_PORT_L=${GAIA_PORT_L:-8462}
CLIENT_PORT=${CLIENT_PORT:-5197}
...
EOF
  ...
  down)
    ...
    echo "lane down"
    ;;
  *) echo "usage: gesture-fix-stack.sh [up|down]"; exit 1;;
esac
