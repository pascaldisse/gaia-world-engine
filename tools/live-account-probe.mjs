...
const tgt=await page.evaluate(()=>{const e=[...document.querySelectorAll('button')].filter(x=>x.offsetParent&&/witness the beginning/i.test(x.textContent||''));if(!e.length)return null;const r=e[0].getBoundingClientRect();return {x:r.x+r.width/2,y:r.y+r.height/2,text:e[0].textContent.trim().slice(0,40)};});
